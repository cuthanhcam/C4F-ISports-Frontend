...
const FieldBooking = () => {
  ...
  const toggleSlotSelection = (subFieldId: number, slot: Slot) => {
    if (slot.status !== "Available") return; // Không chọn slot đã đặt
  ...
  try {
    ...
    // Cập nhật trực tiếp trạng thái slot trong UI
    setFieldData((prev) => {
      if (!prev) return prev;
    ...
    <div className="grid grid-cols-3 gap-3 mt-2">
      {subField.slots.map((slot) => {
        ...
        <button
          ...
          disabled={Object.values(selectedSlots).every((slots) => slots.length === 0)}
        >
          Xác nhận đặt sân
        </button>
      </div>
