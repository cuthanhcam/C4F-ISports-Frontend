                ...
                <div className="relative container py-6 z-[0]">
                    ...
                    <div className="my-12 md:my-20">
                        <div className="relative z-[0] max-w-4xl mx-auto flex flex-col items-center gap-4 border border-outline-variant p-8 rounded-3xl shadow-xunit">
                            <div className="home-header-light-blue"/>
                            <div className="home-header-light-pink"/>
                            <h1 className="text-5xl text-surface-on text-center font-bold">Dịch Vụ C4F-ISports</h1>
                            <h2 className="text-3xl text-surface-inverse text-center font-semibold">Giới Thiệu Chung</h2>
                    ...
                    {/* Các dịch vụ chính */}
                    <div className="my-12 md:my-20">
                        <h1 className="text-5xl text-surface-on text-center font-bold mb-16">Các dịch vụ chính</h1>
                                ...
                                {serviceServiceMainCol1.map((item) => (
                                    <div key={item.id} className="bg-surface-1 p-4 rounded-3xl flex items-start gap-4 shadow-lg">
                                        ...
                                        <ul className="flex flex-col gap-4">
                                            <h1 className="text-3xl text-surface-on font-medium">{item.title}</h1>
                                            <div className="flex flex-col gap-2">
                                                {item.content.map((line, index) => (
                                                    <li key={index} className="text-base text-surface-onVariant leading-relaxed">{line}</li>
                                                ))}
                                            </div>
                                        </ul>
                                    </div>
                    ...
                    {/* Ưu điểm nổi bật */}
                    <div className="my-12 md:my-20">
                        <h1 className="text-5xl text-surface-on text-center font-bold mb-16">Ưu điểm nổi bật</h1>
                        <motion.div variants={containerVariants} animate="animate" className="grid grid-cols-4 gap-4">
                            {serviceAdvantage.map((item, index) => (
                                <motion.div 
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <h1 className="text-xl text-surface-on font-medium">{item.title}</h1>
                                        <p className="text-sm text-surface-onVariant">{item.description}</p>
                                    </div>
                                </motion.div>
                            ))}
                        </motion.div>
                    ...
                    {/* Cam kết dịch vụ */}
                    <div className="my-12 md:my-20">
                        <h1 className="text-5xl text-surface-on text-center font-bold mb-16">Cam kết dịch vụ</h1>
                    </div>
                </div>
            </div>
        </Layout>
