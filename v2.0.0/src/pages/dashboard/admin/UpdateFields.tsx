...
const UpdateFields = () => {
  ...
  useEffect(() => {
    const fetchField = async () => {
      try {
        const response = await fieldsAPI.getFieldsDetail(Number(fieldId));
        setFieldData(response.data);
      } catch (err: any) {
        toast.error("Không thể tải dữ liệu sân: " + (err.message || "Lỗi không xác định"));
      }
    };
  ...
  const handleChange = (
    ...
    setFieldData((prev) => ({
      ...
      [name]:
        ...
        name === "latitude" ||
        name === "longitude" ||
        name === "minPricePerSlot" ||
        name === "maxPricePerSlot"
          ? Number(value)
          : value,
    }));
  };
  ...
  const handleAddSubField = () => {
    ...
    setFieldData((prev) => {
      const newSubFields = [...prev.subFields, subField];
      const minPrice = Math.min(
        ...newSubFields.flatMap((sf) =>
          sf.pricingRules.flatMap((pr) =>
            pr.timeSlots.map((ts) => ts.pricePerSlot)
          )
        )
      );
      ...
      return {
        ...
        minPricePerSlot: minPrice,
        maxPricePerSlot: maxPrice,
      };
    });
    setSubField({
      subFieldName: "",
  ...
  const handleDeleteSubField = (index: number) => {
    setFieldData((prev) => {
      const newSubFields = prev.subFields.filter((_, i) => i !== index);
      const minPrice = newSubFields.length
        ? Math.min(
            ...newSubFields.flatMap((sf) =>
              sf.pricingRules.flatMap((pr) =>
                pr.timeSlots.map((ts) => ts.pricePerSlot)
        ...
        : 0;
      const maxPrice = newSubFields.length
        ? Math.max(
            ...newSubFields.flatMap((sf) =>
              sf.pricingRules.flatMap((pr) =>
                pr.timeSlots.map((ts) => ts.pricePerSlot)
  ...
  const handleAddService = () => {
    ...
    setService({ serviceName: "", price: 0, description: "", isActive: true });
  };
  ...
  const handleDeleteImage = (index: number) => {
    setFieldData((prev) => ({
      ...
      images: prev.images.filter((_, i) => i !== index).map((img, idx) => ({
        ...img,
        isPrimary: idx === 0, // Gán ảnh đầu tiên làm chính
      })),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fieldData.latitude === 0 || fieldData.longitude === 0) {
      toast.error("Vui lòng nhập kinh độ và vĩ độ hợp lệ!");
      return;
    }
    ...
    try {
      await fieldsAPI.updateField(fieldData.fieldId, payload);
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <input
            type="number"
            name="minPricePerSlot"
            value={fieldData.minPricePerSlot}
            onChange={handleChange}
            placeholder="Giá tối thiểu mỗi khung giờ"
            className="p-2 border rounded"
            min="0"
            disabled
          />
          <input
            type="number"
            name="maxPricePerSlot"
            value={fieldData.maxPricePerSlot}
            onChange={handleChange}
            placeholder="Giá tối đa mỗi khung giờ"
            className="p-2 border rounded"
            min="0"
            disabled
          />
        ...
        <select
          ...
          onChange={(e) => setSubField({ ...subField, status: e.target.value })}
          className="w-full p-2 border rounded"
        >
        ...
        {fieldData.subFields.length > 0 && (
          <ul className="mt-2 list-disc list-inside text-gray-700">
            {fieldData.subFields.map((sf, idx) => (
              <li key={idx}>
                {sf.subFieldId ? `(ID: ${sf.subFieldId}) ` : ""}{sf.subFieldName} ({sf.fieldType}) - {sf.status}
                <br />
                <small>{sf.description}</small>
                <button
                  type="button"
                  onClick={() => handleDeleteSubField(idx)}
        ...
        {fieldData.services.length > 0 && (
          <ul className="mt-2 list-disc list-inside text-gray-700">
            {fieldData.services.map((s, idx) => (
              <li key={idx}>
                {s.fieldServiceId ? `(ID: ${s.fieldServiceId}) ` : ""}{s.serviceName} - {s.price} VNĐ ({s.isActive ? "Kích hoạt" : "Không kích hoạt"})
                <br />
        ...
        {fieldData.amenities.length > 0 && (
          <ul className="mt-2 list-disc list-inside text-gray-700">
            {fieldData.amenities.map((a, idx) => (
              <li key={idx}>
                {a.fieldAmenityId ? `(ID: ${a.fieldAmenityId}) ` : ""}
            ...
            {fieldData.images.map((img, idx) => (
              <div key={idx} className="relative">
                ...
                <button
                  ...
                  className="absolute top-0 left-0 bg-red-600 text-white text-xs px-1 rounded"
                >
                  Xóa
                </button>
              </div>
